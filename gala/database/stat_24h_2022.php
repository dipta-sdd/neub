<?php
    ...
    $sql="select COUNT(player) as score,player from email2024 where player is not null and task2='true' AND `up_time`>= NOW() - INTERVAL 1 DAY GROUP by `player`;";
    ...
    $sql="select COUNT(player) as score from email2024 where player is not null and task2='true' AND `up_time`>= NOW() - INTERVAL 1 DAY;";
    $result = $conn->query($sql);
    while($row = $result->fetch_assoc()) {
